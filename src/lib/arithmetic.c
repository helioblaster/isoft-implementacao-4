...
/* Modern GCC combines unsigned division and remainder into this helper.
   Reuse the freestanding divider: no host libgcc is linked into Pintos. */
...
__udivmoddi4 (unsigned long long n, unsigned long long d,
             unsigned long long *remainder)
{
  unsigned long long quotient = udiv64 (n, d);
  if (remainder != 0)
